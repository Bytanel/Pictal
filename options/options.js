...
function addSieve(sieve_name, sieve) {
    ...
    sieve_config.innerHTML = `
        <span id="sieve_title" contenteditable="false">${sieve_name}</span>
        <div>
            <div class="rule_line">
                <label>Link Regex:</label>
                <textarea id="link_regex" wrap="soft" autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false" style="height: 30px;">${sieve.link_regex || ""}</textarea>
            </div>
            <div class="rule_line">
            ...
            <div class="rule_line">
                ...
                <textarea id="image_regex" wrap="soft" autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false" style="height: 30px;">${sieve.image_regex || ""}</textarea>
            </div>
            <div class="rule_line">
        ...
        <div class="action_buttons">
            <span id="rename" title="Rename sieve">✏️</span><span id="disable" title="Disable sieve">🚫</span><span id="delete" title="Delete sieve">🗑️</span>
        </div>
    `;

    ["link_request_javascript", "link_parse_javascript", "image_parse_javascript"].forEach(m => {
    ...
    sieve_config.querySelector("#disable").onclick = function() {
...
function getSieves(search = ""){
    const sieves = document.querySelectorAll(".sieve");
    ...
    for (var i = 0; i < sieves.length; i++) {
        ...
        let new_sieve = {
            ...
            link_request_javascript: link_request_javascript,
            link_parse_javascript: link_parse_javascript,
            image_regex: image_regex,
            image_parse_javascript: image_parse_javascript,
...
async function checkUserScripts() {
    ...
    try {
        ...
        if (scripts?.length > 0) {
            ...
            chrome.runtime.sendMessage({type: "ReloadScript"});
        }
    } catch(e) {
