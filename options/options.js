...
function addSieve(sieve_name, sieve) {
	...
	["link_regex", "image_regex"].forEach(m => {
		...
		if (m == "link_regex") editor.setValue(sieve.link_regex || "", -1);
		if (m == "image_regex") editor.setValue(sieve.image_regex || "", -1);
	...
	["modify_headers_json"].forEach(m => {
		...
		if (m == "modify_headers_json") editor.setValue(beautify(sieve.modify_headers_json || ""), -1);
		const observer = new ResizeObserver(() => {
			editor.resize();
		});
		observer.observe(editor.container);
...
function getSieves(search = "", sieve = null) {
	...
	for (var i = 0; i < sieves.length; i++) {
		...
		const modify_headers_json = ace.edit(sieve.querySelector("#modify_headers_json")).getValue();
		...
		let new_sieve = {
			...
			modify_headers_json: modify_headers_json.trim(),
			notes: notes.trim()
		};
